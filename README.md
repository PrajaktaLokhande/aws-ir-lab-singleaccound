...
- **Billing alerts**:  
  - Set up **AWS Free Tier Usage Alerts** in the [Billing console](https://console.aws.amazon.com/billing/home).
...
1. **Create Stack**
   ...
   - Upload `cloudformation/ir-demo-lab.yaml`.
   - Set parameters (email, IP, schedule, etc.).
...
3. **Watch** for “IAM Key Remediation” email; old key becomes **Inactive**. If `AutoCreateNewIamKey=true`, the new key is stored in **Parameter Store**.
...
- **Remediation didn’t trigger**: Verify EventBridge rule matched (check **CloudWatch Logs** for the Lambda).
